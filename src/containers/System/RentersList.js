...
import { apiAcceptRenter } from "../../services";
...
const RentersList = ({ setShowRenters }) => {
  ...
  const handleAccepter = async (renterId) => {
    ...
    if (response.statusCode === 200) dispatch(actions.getRenters(posts[0]?.id));
  };

  console.log(renters);
  // useEffect(() => {
  //   dispatch(actions.getRenters(posts[0]?.id));
  // }, [renters]);
  ...
  return (
    ...
    >
      ...
      >
        {renters?.filter((renter) => !renter.isConfirmed) &&
        renters?.filter((renter) => !renter.isConfirmed).length > 0 ? (
          <div className="p-6">
            ...
            <table className="my-2 w-full table-fixed">
              ...
              <tbody>
                {renters &&
                  Object.values(renters)[0] !== "" &&
                  renters
                    ?.filter((renter) => !renter.isConfirmed)
                    ?.map((renter) => (
                      <tr
